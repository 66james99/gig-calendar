...
export function renderEditRow(tbody, alias, isNew, promoters) {
    ...
    const promoterOptions = sortedPromoters.map(p => `<option value="${p.ID}" ${alias.Promoter === p.ID ? 'selected' : ''}>${p.Name}</option>`).join('');
    row.innerHTML = `
        <td>${alias.ID || 'New'}</td>
        <td>
...
function getPromoterName(promoterID, promoters) {

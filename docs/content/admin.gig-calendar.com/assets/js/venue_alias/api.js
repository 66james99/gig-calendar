...
export async function fetchVenueAliases() {
...
export async function createVenueAlias(payload) {
    const response = await fetch(`${API_PREFIX}/venue_aliases`, {
        method: 'POST',
        headers: {
...
export async function updateVenueAlias(id, payload) {
...
export async function deleteVenueAlias(id) {
    const response = await fetch(`${API_PREFIX}/venue_aliases/${id}`, {
        method: 'DELETE',
    });
    if (!response.ok) {

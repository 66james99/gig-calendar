...
import type { PromoterAlias, PromoterAliasPayload, PromoterAliasSortableColumn } from './types.js';
...
export async function handleTableClick(event: Event) {
    ...
    // --- Save button (for updating) ---
        ...
        const payload: PromoterAliasPayload = {
            promoter_id: parseInt((row.querySelector('.edit-promoter_id') as HTMLSelectElement).value, 10),
        ...
        try {
            ...
            if (alias) renderDisplayRow(tableBody, alias, promotersCache); // Revert on failure
        }
    }
...
export function handleNewClick(prefill?: Partial<PromoterAlias>) {
    ...
    const newAliasData: Partial<PromoterAlias> = prefill || {};
    renderEditRow(tableBody, newAliasData, true, promotersCache); // Render the row in edit mode.
}

export function handleEditItem(item: PromoterAlias) {
...
export function handleSort(event: Event) {
    ...
    const sortColumn = target.dataset.col as PromoterAliasSortableColumn;

import type { ImageLocation, SortState } from './types.js';

export function renderDisplayRow(row: HTMLTableRowElement, location: ImageLocation) {
    row.innerHTML = `
        <td>${location.ID}</td>
        <td>${location.Root}</td>
...
export function renderEditRow(row: HTMLTableRowElement, location: Partial<ImageLocation>, isNew: boolean = false) {
...
export function renderTable(tableBody: HTMLTableSectionElement, locations: ImageLocation[]) {
    tableBody.innerHTML = '';

...
import type { StageRole, StageRoleSortableColumn, SortDirection } from './types.js';
...
// State
export let stageRolesCache: StageRole[] = [];
export let currentSort: { column: StageRoleSortableColumn, direction: SortDirection } = { column: 'Pattern', direction: 'asc' };
...
export function setCurrentSort(sort: { column: StageRoleSortableColumn, direction: SortDirection }) {
    currentSort = sort;
}
...
export async function init() {
    ...
    if (refreshBtn) refreshBtn.addEventListener('click', refreshStageRoles);
...
export function applyFilters(items: StageRole[]): StageRole[] {
    return items.filter(item => {
        return (
            (item.ID != null ? item.ID.toString() : '').includes(currentFilters.id) &&
...
export function sortStageRoles(items: StageRole[]): StageRole[] {
